...
class DenialConstraint:
    ...
    def __init__(self, dc_string, schema):
        ...
        for p in self.predicates:
            ...
            # check rewritten... Lan
            print(f'rewrite DC in constraint.py : {p.components[0][1]}')

        # Create CNF form of the DC
        cnf_forms = [predicate.cnf_form for predicate in self.predicates]
        print(f'whats CNF form? check from constraint.py: {cnf_forms}')
        self.cnf_form = " AND ".join(cnf_forms)
        print(f'whats self.CNF form? check from constraint.py: {self.cnf_form}')
        print('check from constraint.py')
