...
class DomainEngine:
    ...
    def compute_correlations(self):
        """
        compute_correlations memorizes to self.correlations; a data structure
    ...
    def _compute_norm_cond_entropy_corr(self):
        ...
        for x in attrs:
            corr[x] = {}
            x_vals = data_df[x]
            print(f'the attribute is {x} and the value is {x_vals}')
            x_domain_size = x_vals.nunique()
            # check...what's domain size ...Lan
            print(f'the domain size is from domain.py : {x_domain_size}')
            for y in attrs:
                ...
                print(f'attribute 1 {x} ==> attribute 2 {y}: correlations {corr[x][y]}')
        return corr
    ...
    def get_active_attributes(self):
        ...
        # check ...Lan
        print(f'get active attributes, get the attribute has error from domain.py: {result}')
        if not result:
    ...
    def get_corr_attributes(self, attr, thres):
        ...
        # print(f'what"s current correlated attributes get_corr_attributes from domain.py {self._corr_attrs[(attr, thres)]}')
        print(f'corr_attrs : {self._corr_attrs}')
    ...
    def generate_domain(self):
        ...
        if self.env['weak_label_thresh'] == 1 and self.env['domain_thresh_2'] == 0:
            return domain_df
        domain_df.to_csv('usecase1_domain.csv')
        ...
        for preds, row in tqdm(zip(preds_by_cell, domain_df.to_records())):
            ...
            print(f'weak label: {weak_label}; ==> weak label probability: {weak_label_prob}')
