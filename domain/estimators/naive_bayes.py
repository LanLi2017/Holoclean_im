...
class NaiveBayes(Estimator):
    ...
    def predict_pp(self, row, attr, values):
        ...
        for val1 in values:
            ...
            for at in correlated_attributes:
                ...
                log_prob += math.log(p)
                print(f'attribute 1 {val1} : attribute 2 {val2} ==> probability {log_prob}')
            nb_score.append((val1, log_prob))
            print(f'attribute 1 nbscore: {nb_score}')
    ...
    def _get_corr_attributes(self, attr):
        ...
        print(f'after prune: correlated attributes: {self._corr_attrs}')
