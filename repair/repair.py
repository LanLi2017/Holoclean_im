...
class RepairEngine:
    ...
    def get_infer_dataframes(self, infer_idx, Y_pred):
        ...
        print(f'pos-values from repair.py: {pos_values} ')
        # dict mapping _vid_ --> val_idx --> value
        vid_to_val = {}
        for vid, val_idx, val in pos_values:
            # print(f'pos-values from repair.py vid: {vid} ==> val_idx : {val_idx} ==> val: {val}')
        ...
        for idx in range(Y_pred.shape[0]):
            ...
            print(f'all possible infer values in get_infer_df from repair.py: {rv_val} ==> {Y_pred[idx].data.numpy()}')
            rv_prob = Y_pred[idx].data.numpy().max() # this is for compare?
        ...
        distr_df.to_csv('distribution_df.csv')
        infer_val_df = pd.DataFrame(data=infer_val)
        infer_val_df.to_csv('infer_val_df.csv')
